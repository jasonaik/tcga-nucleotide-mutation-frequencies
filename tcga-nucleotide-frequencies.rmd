...
all_topten <- data.table(mutation = character(), n = integer())
...
for (i in seq_along(cancer_data$Study_Abbreviation)) {
  ...
  topten_nucleotides <- patient_mutation[, .(n = .N), by = mutation][order(-n)][1:top_num]

  all_topten <- rbind(all_topten, topten_nucleotides)
...
all_topten <- all_topten[, .(n = sum(n)), by = mutation][order(-n)]

# Find hits
patient_hits <- all_unique_patient_data[, hits := sapply(mutations, function(x) length(intersect(x, all_topten$mutation)))][order(-hits)]

fraction_covered <- patient_hits[hits > 0, .N] / patient_hits[, .N]
...
coverage_per_cancer_plot <- ggplot(coverage_per_cancer, aes(x = cancer_type, y = coverage)) +
  geom_bar(stat = "identity", fill = "blue", width = 0.9) +
