...
optimized_setdiff <- function(x, y) {
...
greedy_panel_curator <- function(patient_data, top_number) {
  ...
  for (i in 1:top_number) {
    ...
    panel <- rbind(panel, new_panel_row, use.names = TRUE, fill = TRUE)
    ...
    # Use mclapply to remove patients in parallel
    if (length(patients_with_best_mutation) > 0 && nrow(mutation_patient_list) > 0) {
      # mutation_patient_list[, patients := lapply(patients, setdiff, y = patients_with_best_mutation)]
      mutation_patient_list$patients <- mclapply(
        mutation_patient_list$patients, optimized_setdiff,
        y = patients_with_best_mutation, mc.cores = num_cores
      )
      mutation_patient_list <- mutation_patient_list[lengths(mutation_patient_list$patients) > 0]
    }

    # Update mutation coverage after removing patients
    if (nrow(mutation_patient_list) > 0) {
      mutation_coverage <- mutation_patient_list[, .(mutation, count = lengths(patients))][order(-count)]
    }
  }
...
#     # Remove the best mutation and patients with this mutation from the list
#     mutation_patient_list <- mutation_patient_list[!(mutation == best_mutation)]
#     # mutation_patient_list$patients <- mclapply(mutation_patient_list$patients, setdiff, y = patients_with_best_mutation, mc.cores = num_cores_to_use)
#     # mutation_patient_list <- mutation_patient_list[lengths(mutation_patient_list$patients) > 0]
#     mutation_patient_list[, patients := lapply(patients, optimized_setdiff, y = patients_with_best_mutation)]
...
# profvis(panel <- greedy_panel_curator(specific_patients, max(top_nums)))
...
pdf("plots/cancer-specific-panel-coverage-greedy.pdf")
