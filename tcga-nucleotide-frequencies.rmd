...
cs_dis_plot <- test_and_plot_cs(final_panel, patient_data_training, "Cancer Specific Panel Coverage (On Discovery)", "cs-tops-pluspc-ondis", "Set2", TRUE)
cs_test_plot <- test_and_plot_cs(final_panel, patient_data_test, "Cancer Specific Panel Coverage (On Test)", "cs-tops-pluspc-ontest", "Set2", TRUE)
...
# ## STRANGER DANGER - actually we need both - run code twice probs
...
# Gene-cancer heatmap
gene_panel_hits_complete[order(gene)]

gene_colormap_plot <- ggplot(gene_panel_hits_complete, aes(gene, cancer_type, fill = top_number)) +
  geom_tile(color = "white") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2", direction = -1, na.value = "white") +
  labs(
    x = "Cancer Type",
    y = "Gene"
  ) +
...
mutation_panel_hits_complete[order(mutation)]

mutation_colormap_plot <- ggplot(mutation_panel_hits_complete, aes(mutation, cancer_type, fill = top_number)) +
  geom_tile(color = "white") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2", direction = -1, na.value = "white") +
  labs(
...
added_legend_gene <- plot_grid(
  added_gene_heatmap, plot_legend,
...
added_legend_mutation <- plot_grid(
  added_mutation_heatmap, plot_legend,
  ncol = 2, rel_widths = c(7, 1), scale = 0.95
)
...
pan_can_tops_dis_plot <- test_and_plot_pc(final_panel_tops, patient_data_training, "Pan-Cancer Panel (On Discovery)", "pan-can-tops-cved-ondis", "Set2")
pan_can_tops_test_plot <- test_and_plot_pc(final_panel_tops, patient_data_test, "Pan-Cancer Panel (On Test)", "pan-can-tops-cved-ontest", "Set2")
