...
```{r top-mutations-cross-validation-cancer-specific}
cv_fraction <- 0.8
...
final_panel_raw <- drop_count[, .SD[1:min(100, .N)], by = cancer_type]
final_panel_raw <- final_panel_raw[, .(panel = list(mutation)), by = cancer_type]
...
# sample 80% of training data
unique_specific_patients <- specific_patients[, unique(patient)]
eighty_training_patients <- sample(unique_specific_patients, round(length(unique_specific_patients) * 0.8))
...
for (num in top_nums) {
  ...
  new_panel <- data.table(top_number = num, panel = list(current_panel))
  training_pan_can <- rbind(training_pan_can, new_panel)
}

fwrite(training_pan_can, "csv-data/training_pan_can_coverage.csv")
```
